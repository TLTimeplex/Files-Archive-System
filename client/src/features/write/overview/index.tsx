        ...
        <tbody>
          {
            Reports.map((report, index) => {
              return (
                <ListItem key={index} Title={report.Report.title || ""} isArchived={false} statusSet={{ isLocallyAvailable: report.isLocal || report.isRemote, isSynced: report.isSynced, isPublic: false, hasConflicts: (!report.isSynced && report.isRemote) }} date={report.Report.updatedAt} content={report.Report.report !== undefined ? report.Report.report?.slice(0, 50) + "..." : ""} viewCallback={async () => { if (report.canSync && isOnline) await syncReport(report.Report.id); document.location.href = "/report/edit/" + report.Report.id }} deleteCallback={/* TODO: Check if is own */ () => { deleteReport(report.Report.id) }} syncCallback={report.canSync && isOnline ? () => syncReport(report.Report.id) : undefined} mergeCallback={report.canMerge && isOnline ? () => {/*TODO:*/ } : undefined} archiveCallback={isOnline ? () => archiveReport(report.Report.id) : () => { }} />
              );
            })
          }
        </tbody>
